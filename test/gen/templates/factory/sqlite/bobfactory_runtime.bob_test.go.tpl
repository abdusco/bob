...
func TestUserSliceReloadAll(t *testing.T) {
	...
	// Store original IDs
	originalIDs := make([]int64, len(users))
	for i, u := range users {
		originalIDs[i] = u.ID
	}
...
func TestPreloadCountWithFilter(t *testing.T) {
	...
	// Create 3 videos for user1 and 2 for user2
	for i := 0; i < 3; i++ {
		New().NewVideoWithContext(ctx,
			VideoMods.WithExistingUser(user1),
		).CreateOrFail(ctx, t, tx)
	...
	// Query user1 with PreloadCount filtering for videos belonging to user1
	users, err := models.Users.Query(
	...
	if *users[0].C.Videos != 3 {
		t.Fatalf("Expected filtered Videos count to be 3, got %d", *users[0].C.Videos)
	}
}
...
func TestQueryWithWhere(t *testing.T) {
	...
	// Create two users with different numbers of videos
	user1 := New().NewUserWithContext(ctx).CreateOrFail(ctx, t, tx)
	user2 := New().NewUserWithContext(ctx).CreateOrFail(ctx, t, tx)
	...
	// Query videos using Where clause for user1
	...
	if len(videos) != 5 {
		t.Fatalf("Expected 5 videos for user1, got %d", len(videos))
	}
	...
	if len(videos) != 2 {
		t.Fatalf("Expected 2 videos with limit, got %d", len(videos))
	}
}
...
func TestDeleteWithMods(t *testing.T) {
	...
	// Create two users with videos
	user1 := New().NewUserWithContext(ctx).CreateOrFail(ctx, t, tx)
	user2 := New().NewUserWithContext(ctx).CreateOrFail(ctx, t, tx)

	// Create 5 videos for user1
	for i := 0; i < 5; i++ {
		New().NewVideoWithContext(ctx,
			VideoMods.WithExistingUser(user1),
		).CreateOrFail(ctx, t, tx)
	...
	// Verify user2 still has 3 videos
	count, err = models.Videos.Query(
		models.SelectWhere.Videos.UserID.EQ(user2.ID),
	).Count(ctx, tx)
	if err != nil {
...
func TestSelectJoins(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("Error querying videos with InnerJoin: %v", err)
	}
